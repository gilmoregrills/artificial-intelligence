...
class MoreDefenceD4RobinPlayer160719011 extends GomokuPlayer {
	...
	public MoveScore alphaBeta(Color[][] board, Color me, int depth, int alpha, int beta, boolean max) {
		...
		if (max == true) {
			...
			for (MoveScore currentMove : moves) {
				...
				//pruning - still not sure about this but it seems to be working?
				if (beta <= alpha) {
		...
		for (int i = 0; i < 8; i++) {//rows
			for (int j = 0; j < 8; j++) {//cols
				if (board[i][j] == Color.white) {
					...
					boardScore += (Color.white == me) ? squareScore : squareScore*1.04;//opponent score multiplier
				} else if (board[i][j] == Color.black) {
					squareScore = scorePatterns(getPatterns(board, Color.black, i, j));
					boardScore -= (Color.black == me) ? squareScore : squareScore*1.04;//encourages more or less defensive play
				}
			}
		}
